...
const GROQ_KEY =
  process.env.VITE_GROQ_API_KEY_LESSON ||
  process.env.VITE_GROQ_API_KEY_CHAT ||
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    try {
      const text = await (Math.random() > 0.5
        ? callGroq(systemInstruction, userPrompt)
        : callPollinations(systemInstruction, userPrompt));
